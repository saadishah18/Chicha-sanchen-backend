...
@section('content')
    <div class="container-fluid">
        <h1 class="h3 mb-2 text-gray-800">Plaza Extra East</h1>
        @if (session('status'))
        ...
        <div class="card-header pl-0 py-3 right">
            @can('manage_plaza_extra_east_section_content')
                <a href="{{ route('admin.manage_plaza.create') }}" class="btn btn-info btn-icon-split">
                    <span class="icon text-white-50">
                        <i class="fas fa-plus"></i>
                    </span>
                    <span class="text">Add Plaza Extra East</span>
                </a>
            @endcan
        </div>
        <nav>
            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                <a class="nav-item nav-link active" id="tab1" data-toggle="tab" href="#nav-home" role="tab"
                    aria-controls="nav-home" aria-selected="true">Virgin Island Plaza Extra East</a>
                <a class="nav-item nav-link" id="tab2" data-toggle="tab" href="#nav-profile" role="tab"
                    aria-controls="nav-profile" aria-selected="false">Caribbean Plaza Extra East</a>
            </div>
        </nav>
        <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="tab1">
                @include('admin.pages.plaza.datatable', ['id' => 'datatable1'])
            </div>
            <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="tab2">
                @include('admin.pages.plaza.datatable', ['id' => 'datatable2'])
            </div>
        </div>
...
@section('js')
    ...
    <script type="text/javascript">
        function toggleActivity(e, column = 'is_breaking_news') {
            ...
            url = '/admin/manage-plaza/' + itemId + '/toggle';
            Swal.fire({
                // title: 'Are you sure?',
        ...
        function initializeDatatable(tableId, newsTypeId) {
            url = `/admin/manage-plaza/datatable/${newsTypeId}`
            $(`#${tableId}`).DataTable({
                pageLength: 25,
                processing: true,
                serverSide: true,
        ...
        function confirmBeforeDelete(tablesToResetFunction) {
            $(document).on('click', '.delete_item_from_list', function(e) {
                ...
                url = '/admin/manage-plaza/delete/' + postId;
                Swal.fire({
                    title: 'Are you sure?',
