...
@section('content')
    <div class="container-fluid">
        ...
        @include('admin.pages.partials.back_arrow',['backRoute' => route('admin.manage_plaza.index'),'text' => 'Add Plaza Extra East'])
        <div class="col-lg">
            <form method="post" action="{{ route('admin.manage_plaza.store') }}"  enctype="multipart/form-data">
                @csrf
                @if (session('status'))
                ...
                @include('admin.components.form.input',['fieldId' => 'title','fieldTitle' => 'Plaza extra east title', 'placeholder' => 'Enter plaza extra east name','required' => true, 'autofocus' => true,'labelCols' => 'col-sm-3','inputCols' => 'col-sm-9'])
                <div class="form-group row">
                    <label class="col-sm-3 col-form-label">Plaza extra east Image</label>
                    <div class="col-sm-9">
                        <img class="img-thumbnail" src="" onerror="this.onerror=null; this.src=this.src=this.style.display = 'none'" alt="plaza_extra_east_img" id="plaza_extra_east_img_preview">
