...
<a title="{{ $product->is_featured == 1 ? 'Featured' : 'Remove Feature' }} product" data-id="{{ $product->id }}" href="#"
...
<a title="{{ $product->is_active ? 'Deactive' : 'Active' }} product" data-id="{{ $product->id }}"  data-status="{{ $product->is_active ? 'Active' : 'Inactive' }}"  href="#"
   class="btn {{ $product->is_active ? 'btn-success' : 'btn-danger' }} btn-circle deactivate-product">
