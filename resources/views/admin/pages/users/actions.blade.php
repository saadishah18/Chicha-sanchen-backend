@can('edit_users')
    <a title="Edit user" href="{{ route('admin.users.edit', ['id' => $id]) }}" class="btn btn-info btn-circle">
...
@if (auth()->check() && auth()->user()->user_id != $user->user_id)
    @can('approve_button')
        <a title="{{ $user->approve ? 'Deactive' : 'Active' }} user" data-id="{{ $id }}" href="#"
    ...
    @endcan
    @can('delete_users')
        <a href="#" title="Delete user" data-id="{{ $id }}"
